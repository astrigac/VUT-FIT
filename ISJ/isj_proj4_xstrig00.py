...
def match_permutations_substrings(string, words):
    ...
    return {word for word in words if word in perms}                                       # your solution
...
def uniq_orig_order(it):
    ...
    return list(seen.add(item) or item for seen, item in [(set(), None)] for item in it if item not in seen)       # your solution
