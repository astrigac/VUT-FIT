...
def gen_quiz(qpool, *indexes, altcodes='ABCDEF', quiz=None):
    ...
    for index in indexes:
        try:
            ...
            formatted_answers = [f"{code}: {ans}" for code, ans in zip(altcodes, answers)]
            quiz.append((question, formatted_answers))
        except IndexError as index_error:
            print(f"Ignoring index {index} - {str(index_error)}")
        except Exception as e:
            print(f"Ignoring index {index} - unexpected error: {str(e)}")
